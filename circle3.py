...
def detect_rain(image_path, line_notify_token1):
    ...
    points_to_detect = [
        ...
        (400, 457, 18, "พล"),
        (200,300,150, "สวัสดีครับอันนี้คือไว้สร้างจดใหม่ 200 คือ x,y ส่วน 150 คือ รัศมีวงกลมครับ")
    ]
    ...
    for center_x, center_y, radius, name in points_to_detect:
        ...
        if red_area > 4:
            ...
            message += f"\nพื้นที่ของฝนสีแดงที่ {name}: {red_area} Pixel"
    ...
    if cropped_images:
        ...
        combined_image_bytes = encoded_image.tobytes()

        # ส่งข้อความพร้อมรูปผ่าน Line Notify
        send_line_notify_with_cropped_image(message, line_notify_token1, combined_image_bytes)
        time.sleep(0.5)
    # else:
...
def download_and_detect():
    while True:
        ...
        while True:
            ...
            if current_time.endswith("14") or current_time.endswith("20") or current_time.endswith("35") or current_time.endswith("50"):
